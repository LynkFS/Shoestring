...
//JAnchor
  ...
  ComponentRec.ShowIt := procedure begin
    {--------------------------------------------------------------------------}
    { Component  JW3Anchor (url link)                                          }
    { Updated:   01/09/2017                                                    }
    {--------------------------------------------------------------------------}
    var Anchor1 := JW3Anchor.Create(DisplayDiv);
...
//JFlipPage
  ...
  ComponentRec.ShowIt := procedure begin
    {--------------------------------------------------------------------------}
    { Component  JW3FlipScroll                                                 }
    { Updated:   01/09/2017                                                    }
    {--------------------------------------------------------------------------}
    var FlipScroll := JW3FlipScroll.Create(DisplayDiv);
    FlipScroll.SetBounds(0, 0, 350, 500);

    var encodestr := window.encodeURIComponent('https://www.symphonyone.com/');
    FlipScroll.setAttribute('src','https://www.lynkfs.com/native/www/lib/pageflip.php?url=' + encodestr);

    var ClickPanel := JW3Panel.Create(DisplayDiv);
...
//JDialog
  ...
  ComponentRec.ShowIt := procedure begin
    ...
    { Updated:   01/09/2017                                                    }
    {--------------------------------------------------------------------------}
    var Dialog1 := JW3Dialog.Create(self); //or application, or form

    var MyButton := JW3Button.Create(DisplayDiv);

    MyButton.SetBounds(0,0,150,40);
    MyButton.Caption := 'Open dialog';
    MyButton.setProperty('background-color', 'blueviolet');
    MyButton.OnClick := procedure(sender:TObject)
    begin
      var Canvas1 := JW3Canvas.Create(Dialog1);
      Canvas1.SetBounds(0, 0, 400, 200);
      Canvas1.ctx.beginPath();
      Canvas1.ctx.moveTo(75, 25);
      Canvas1.ctx.quadraticCurveTo(25, 25, 25, 62.5);
...
//JDBDesignSurface
  ComponentRec := TComponentRec.Create;
  ComponentRec.Name := 'JDBDesignSurface - under constr';
  ComponentRec.ShowIt := procedure begin
    {--------------------------------------------------------------------------}
    { Component  JW3DBDesignSurface                                                     }
    { Updated:   01/09/2017                                                    }
    {--------------------------------------------------------------------------}
    var Window1 := JW3Window.Create(self); //or application, or nil
    ...
    MyButton.SetBounds(0,0,200,40);
    MyButton.Caption := 'Open DBDesignSurface';
    MyButton.OnClick := procedure(sender:TObject)
    begin
      var DBDesignSurface1 := JW3DBDesignSurface.Create(Window1);
      DBDesignSurface1.SetBounds(50, 0, 935, 400);
      Window1.OpenWindow;
    end;
  end;
  Components.Add(ComponentRec);
...
//JWebGrid
  ComponentRec := TComponentRec.Create;
  ComponentRec.Name := 'JWebGrid - under constr.';
  ComponentRec.ShowIt := procedure begin
    {--------------------------------------------------------------------------}
    { Component  JW3WebGrid                                                    }
    { Updated:   01/09/2017                                                    }
    {--------------------------------------------------------------------------}
    var WebGrid1 := JW3WebGrid.Create(DisplayDiv);
    WebGrid1.SetBounds(0, 0, 600, 120);
  end;
  Components.Add(ComponentRec);
*/
...
//JWindow
  ComponentRec := TComponentRec.Create;
  ComponentRec.Name := 'JWindow - under constr.';
  ComponentRec.ShowIt := procedure begin
    {--------------------------------------------------------------------------}
    { Component  JW3Window                                                     }
    { Updated:   01/09/2017                                                    }
    ...
    begin
      ...
      Button1.Caption := 'check first radiobutton';
      Button1.OnClick := procedure(sender:TObject)
      begin
