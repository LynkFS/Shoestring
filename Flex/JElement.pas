...
type
  ...
  public
    ...
    Procedure SetCSS(pseudo, d, prop, value: string); overload;
...
begin
  ...
  FElementStyle.setProperty('flex-direction','column');
  //FElementStyle.setProperty('overflow','auto');
  FElementStyle.setProperty('margin', '0');
  FElementStyle.setProperty('padding', '0');
  FElementStyle.setProperty('box-sizing', 'border-box');
  FElementStyle.setProperty('font-family', '"Segoe UI", Tahoma, Geneva, Verdana, sans-serif');
...
Procedure TElement.SetCSS(pseudo, d, prop, value: string);
begin
//sheet.addRule('#OBJ5:hover','background-color: #4CAF50;');
//sheet.insertRule('#OBJ5:hover { color: green }', 0);
  ...
  //styleSheet.addRule(s0+':'+ pseudo,s1);
  //console.log(s0+':'+ pseudo,s1);
  styleSheet.insertRule(s0 + d + ':' + pseudo + ' { ' + s1 +' }', styleSheet.cssRules.length);     //,0
  //console.log(s0 + '::' + pseudo + ' { ' + s1 + ' }');
end;
